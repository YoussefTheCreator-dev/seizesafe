...
FILES = ["Stand.csv", "Walk.csv", "FastWalk.csv", "Sit.csv", "SitStand.csv", "Fall.csv", "Seizure.csv"]
SAMPLING_RATE = 50  # Hz
WINDOW_SIZE = 256
STRIDE = 128  # 50% overlap

# Label mapping for Wrist (remapped to 0-indexed contiguous integers)
ORIGINAL_LABELS = [0, 1, 2, 3, 4, 6, 8]
LABEL_NAMES = ["Stand", "Walk", "FastWalk", "Sit", "SitStand", "Fall", "Seizure"]
...
def main():
    ...
    for start in range(0, len(full_df) - WINDOW_SIZE + 1, STRIDE):
        ...
        X_raw.append(np.column_stack(raw_window)) # Shape: (256, 8)
    ...
    with open('wrist_label_mapping.json', 'w') as f:
        json.dump(LABEL_MAPPING, f, indent=4)
    joblib.dump(LABEL_MAPPING, 'wrist_label_encoder.pkl')
    ...
    # SMOTE for final RF and XGBoost training
    ...
    for model_name, model in models.items():
        ...
        cv_acc = []
        cv_f1 = []
        # 5-fold CV on training set. For CV, apply SMOTE only to training folds
    ...
    if TF_AVAILABLE:
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(X_raw_train_scaled, y_train)):
            ...
            preds_prob = cnn_lstm.predict(X_fold_val, verbose=0)
        ...
        plt.title('Accuracy')
        plt.tight_layout()
        plt.savefig('wrist_training_history.png')
        plt.close()
        
        preds_prob = final_cnn_lstm.predict(X_raw_test_scaled)
        test_preds = np.argmax(preds_prob, axis=1)
        test_acc = accuracy_score(y_test, test_preds)
        test_f1 = f1_score(y_test, test_preds, average='macro')
        results["CNN-LSTM"] = {"cv_acc": mean_acc, "test_acc": test_acc, "test_f1": test_f1, "preds": test_preds, "model": final_cnn_lstm}
        
        print("\n  Test Set Classification Report:")
        print(classification_report(y_test, test_preds, target_names=LABEL_NAMES))

    print("\n=== Model Comparison ===")
    print(f"{'Model':<15} | {'CV Acc':<8} | {'Test Acc':<8} | {'Test F1':<8}")
    print("-" * 47)
    ...
    # Save artifacts for the best model and tree-based
    joblib.dump(results["Random Forest"]["model"], 'wrist_rf_model.pkl')
    joblib.dump(hc_scaler, 'wrist_rf_scaler.pkl')
    plot_feature_importance(results["Random Forest"]["model"], feature_names, "Top 20 Features - RF (Wrist)", 'wrist_rf_feature_importance.png')

    joblib.dump(results["XGBoost"]["model"], 'wrist_xgb_model.pkl')
    joblib.dump(hc_scaler, 'wrist_xgb_scaler.pkl')
    plot_feature_importance(results["XGBoost"]["model"], feature_names, "Top 20 Features - XGBoost (Wrist)", 'wrist_xgb_feature_importance.png')
    
    if TF_AVAILABLE and "CNN-LSTM" in results:
        results["CNN-LSTM"]["model"].save('wrist_cnn_lstm_model.h5')
        # We save raw_scaler for CNN-LSTM
        joblib.dump(raw_scaler, 'wrist_cnn_lstm_scaler.pkl')
        
    # Save confusion matrix for best model
    plot_confusion_matrix(y_test, results[best_model_name]["preds"], f"Confusion Matrix - {best_model_name} (Wrist)", 'wrist_confusion_matrix.png')
    print("Done!")
