...
def main():
    ...
    # Connect to the /esp32 namespace
    print("\n[WS] Connecting to " + url + " ...")
    try:
        sio.connect(url, namespaces=["/esp32"], transports=["websocket","polling"])
    except Exception as e:
        print("[WS] Connection failed: " + str(e))
        print("\nTroubleshooting:")
        print("  1. Make sure the Railway server is running")
        print("  2. Check the URL is correct: " + url)
        print("  3. Try: python replay_demo_cloud.py --url http://localhost:5000")
        sys.exit(1)

    print("[WS] Waiting for connection...")
    if not connected.wait(timeout=15):
        print("[WS] Timeout — server did not respond")
        sys.exit(1)

    print("[WS] Ready! Starting in 3 seconds...")
    time.sleep(3)
