...
def load_models():
    models = {}
    wfiles = ["wrist_rf_model.pkl","wrist_rf_scaler.pkl","wrist_label_mapping.json"]
    afiles = ["ankle_rf_model.pkl","ankle_rf_scaler.pkl","ankle_label_mapping.json"]
    if all(os.path.exists(f) for f in wfiles):
        models[0] = Pack(*wfiles, CRIT_W, WLABELS)
        print("[Models] Wrist RF loaded")
    else:
        print("[Models] WARNING: Wrist model files missing")
    if all(os.path.exists(f) for f in afiles):
        models[1] = Pack(*afiles, CRIT_A, ALABELS)
        print("[Models] Ankle RF loaded")
    else:
        print("[Models] WARNING: Ankle model files missing")
    if not models:
